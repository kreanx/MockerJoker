...
- Modify ответов (удалить/установить заголовки ответа)
- **Modify Body** — трансформация тела запроса по JSON path
- **Body conditions** — матчинг правил по содержимому тела запроса
- 9 пресетов для типичных сценариев
...
| `modifyResponse` | Изменение ответа: удалить/установить заголовки ответа (тело и статус — настоящие) |
| `modifyBody` | Трансформация тела запроса: заменить значения по JSON path |

## Body Conditions

Любое правило может содержать условия на тело запроса. Правило сработает только если все условия выполняются:
...
Путь поддерживает dot notation с индексами массивов: `signal`, `attrs.items[0].name`.
