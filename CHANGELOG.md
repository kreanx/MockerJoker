...
## v6.0.10 (2026-08-18) — Перезагрузка страницы после правки в точке останова больше не стирает лог

### Исправлено
- **Запрос, отредактированный в точке останова, исчезал из лога при включённой F5-очистке**: когда страница сама перезагружалась сразу после «Продолжить» (типичное поведение приложений после сохранения), `location.reload()` выглядел для Chrome как обычный F5, и лог очищался — вместе с правленым запросом и всеми предыдущими.
- Очистка теперь подавляется, если навигация началась в течение 3 секунд после последней активности точки останова (пауза/продолжить) — это программная перезагрузка страницы, а не пользовательский F5. Пользовательский F5 позже этого окна чистит лог как раньше.
- Дополнительно: вторая `loading`-фаза навигации (редирект) лог не стирает независимо от того, сколько времени прошло.

## v6.0.9 (2026-08-18) — F5-очистка: лог чистится ровно один раз за навигацию

### Исправлено
- **Очистка лога при обновлении страницы срабатывала дважды** — первая партия запросов исчезала, оставалась только вторая. Chrome шлёт второе `loading`-событие и для обычного F5, и при JS-редиректе после полной загрузки; прежний гейт сбрасывался на `complete`, и повторная фаза чистила лог снова.
- Теперь лог чистится только на первой `loading`-фазе каждой навигации; повторная `loading` в течение 3 секунд (вторая фаза той же навигации или редирект) лог не трогает, а новый пользовательский F5 — чистит как раньше.
- Гейт переживает перезапуск service worker (состояние дублируется в `storage.session`) — перезапущенный SW не может «двойно» почистить лог посреди навигации.

## v6.0.8 (2026-08-18) — Правки ответа в точке останова применяются (в т.ч. XHR), $переменные, дифф

### Исправлено
- **Правки в точке останова (фаза ответа) реально применяются для XHR** — раньше модалка показывала поле «Тело ответа» и честно говорила «только fetch»: для XHR `responseText` read-only, правки молча игнорировались. Теперь статус и тело применяются через переопределение свойств инстанса (тот же приём, что у мок-правил) — страница получает изменённый ответ. Ограничение снято, подсказка в модалке обновлена.
- **`$переменные` в теле брейкпоинта** — в отредактированном теле запроса/ответа подставляются сохранённые переменные (`$token` → значение), для fetch и XHR, request и response фаз.
- **F5-очистка лога: однократная** — редирект/вторая loading-фаза навигации больше не стирает первую партию запросов (гейт на первую loading-фазу каждой навигации).

### Добавлено
- **Дифф изменений в детальном виде** — для правок тела — построчный дифф «было → стало» (изменённые строки подсвечены, как у модификаций ответа), для URL/метода/статуса — строки «старое → новое». Значения передаются полностью (тело до 200 КБ), чипы в таблице обрезают только для тултипа.

## v6.0.7 (2026-08-18) — Подсветка изменённых брейкпоинтом + F5-очистка лога

### Добавлено
- **Подсветка строк, изменённых через точку останова**: строка получает цветную подложку (в дополнение к бейджу `BP ✏`), а в колонке «ТЗ» появляются чипы изменений — `✏ url`, `✏ тело`, `✏ статус` — с указанием «было → стало» в тултипе. В детальном виде запроса — таблица «Изменения точки останова» (поле, старое значение, новое).
- **Тумблер «F5-очистка»** в тулбаре панели: при включённом лог перехватов стирается при обновлении страницы, при выключенном — сохраняется (поведение по умолчанию). Состояние хранится в storage и применяется на лету (без перезапуска SW). Заодно починено: Chrome не отдаёт `changeInfo.url` при обычном F5 — обработчик reload теперь завязан только на статус `loading`.

## v6.0.6 (2026-08-18) — Кнопка «Точка» в детальном виде запроса

### Добавлено
- **«Точка»** рядом с «Замокать» в детальном виде запроса — открывает диалог точки останова с полным URL и методом запроса (фаза по умолчанию — request, меняется в диалоге). Тот же путь, что и контекст-меню «Точка останова», но без правого клика.

## v6.0.5 (2026-08-18) — Панель переживает перезапуск service worker

### Исправлено
- **Запросы пропадали из панели после рестарта SW (F5/простой)** — MV3 убивает idle service worker даже при открытом порте (Chrome 114+). Панель держала мёртвый порт: живые апдейты и брейкпоинты не доходили, лог оживал только после переоткрытия DevTools (backlog). Теперь: авто-переподключение порта с экспоненциальным backoff (1–15 с; background шлёт свежий backlog на каждое подключение), дедупликация backlog по id, и keepalive-пинг каждые 20 с — SW живёт, пока открыт DevTools.

## v6.0.4 (2026-08-18) — Критический фикс XHR

### Исправлено
- **`ReferenceError: bpRule is not defined` на каждом XHR** — реворк маркеров брейкпоинтов в v6.0.3 (`c44066f`) потерял четыре строки преамбулы `OrigXHR.prototype.send` (`reqId`, pending-запись, `bpRule`, `_bpSkip`): каждый `xhr.send()` падал до репорта, лог DevTools оставался пустым на XHR-страницах. Восстановлено дословно из v6.0.2; E2E теперь покрывает XHR-путь целиком (send, запись в лог, request-брейкпоинт с правкой в модалке, бейдж `BP ✏`).

## v6.0.3 (2026-08-18) — Живой лог перехватов + столбец «ТЗ»

### Добавлено
- **Столбец «ТЗ» в логе перехватов DevTools-панели** — видно, какие запросы были остановлены точкой останова и что с ними стало: `BP ⏸` ждёт «Продолжить», `BP ✏` изменён, `BP ⛔` отменён, `BP ✓` прошёл без правок. Работает для fetch и XHR, обе фазы (запрос/ответ).
### Исправлено
- **Запросы не появлялись в DevTools-панели «вживую»** — в рефакторе v6.0.1 (27cb929) было удалено определение `broadcastToPanels`, но вызовы остались: каждый живой апдейт лога и keepalive-пинг падали с `ReferenceError`, панель наполнялась только backlog'ом при (пере)подключении. Определение восстановлено.
- **Запросы не попадали в панель до ручного обновления страницы** — вкладки, загрузившиеся раньше пробуждения MV3-расширения (рестарт браузера, восстановление сессии), жили без контент-скрипта. Теперь при подключении DevTools-панели background дозаджектит контент-скрипты в таб (`chrome.scripting.executeScript`); `injected.js` защищён от двойной обёртки fetch/XHR (`__rmWrapped`).
- **Диалог точки останова: звёздочки** — поле URL больше не префиллится `*/api/*`; при создании из контекст-меню запроса подставляется его полный URL, при ручном создании поле пустое с подсказкой «полный URL или маска */api/*».

## v6.0.2 (2026-08-18) — Типизация значений + точные URL из DevTools

### Исправлено
- **«Замокировать» из DevTools больше не оборачивает URL звёздочками** — правило создавалось с паттерном `*host/path*` (и теряло query-string). Теперь в URL-паттерн подставляется полный URL запроса как есть: `https://host/path?query`. Точное совпадение; при необходимости маску можно дописать руками в редакторе.
